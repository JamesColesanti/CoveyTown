...
import { isPosterSessionArea, isViewingArea, isWhiteboardArea } from '../TestUtils';
import {
  ChatMessage,
...
import GalleryCanvas from './WhiteboardArea/GalleryCanvas';
...
export default class Town {
  ...
  async addPlayer(userName: string, socket: CoveyTownSocket): Promise<Player> {
    ...
    socket.on('interactableUpdate', (update: Interactable) => {
      if (isViewingArea(update)) {
        ...
        } else if (isWhiteboardArea(update)) {
          // forward interactableUpdate
          newPlayer.townEmitter.emit('interactableUpdate', update);
          // find an existing poster session area with the same ID
          const existingWhiteboardArea = <WhiteboardArea>(
            this._interactables.find(area => area instanceof WhiteboardArea)
          );
          // updatemodel if theres an existing poster session area with the same ID
          if (existingWhiteboardArea) {
            existingWhiteboardArea.updateModel(update);
          }
        }
      }
    });

    // socket for handling the timing aspect of the canvas
    socket.on('startTimer', () => {
      ...
      const timerInterval = setInterval(() => {
        ...
        if (time === 60) {
          ...
          time = 0;
          this._broadcastEmitter.emit('stopTimer');
          const existingWhiteboardArea = <WhiteboardArea>(
            this._interactables.find(area => area instanceof WhiteboardArea)
          );
          if (existingWhiteboardArea) {
            existingWhiteboardArea.destroyCanvas();
            existingWhiteboardArea.clearComments();
          }
        }
      }, 1000);
    });
  ...
  /**
   * Creates a new whiteboard area in this town if there is not currently an active
   * whiteboard area with the same ID. The whiteboard area ID must match the name of a
   * whiteboard area that exists in this town's map, and the whiteboard area must not
   * already have a poster image set.
   *
   * If successful creating the poster session area, this method:
   *    Adds any players who are in the region defined by the poster session area to it
   ...
   * with the specified ID or if there is no poster image and title specified
   */
  public addCanvas(whiteboardArea: Canvas): boolean {
    // find an existing whiteboard area with the same ID
    const existingWhiteboardArea = <WhiteboardArea>(
      this._interactables.find(area => area instanceof WhiteboardArea)
    );

    // we've reached here -- it's a valid update
    existingWhiteboardArea.createNewCanvas(whiteboardArea);
    existingWhiteboardArea.addPlayersWithinBounds(this._players);
    this._broadcastEmitter.emit('interactableUpdate', existingWhiteboardArea.toModel());
  ...
  public getCurrentCanvas(): Canvas | undefined {
    ...
    return existingWhiteboardArea.getCurrentCanvas();
  }

  public getGallery(): GalleryCanvasModel[] {
    const existingWhiteboardArea = <WhiteboardArea>(
      this._interactables.find(area => area instanceof WhiteboardArea)
    );
    return existingWhiteboardArea.gallery;
  }

  public updateArea(newPixelData: NewPixelData): void {
    // find an existing whiteboard area with the same ID
    const existingWhiteboardArea = <WhiteboardArea>(
      this._interactables.find(area => area instanceof WhiteboardArea)
    );
    ...
    // we've reached here -- it's a valid update
    existingWhiteboardArea.updateCanvas(newPixelData);
    this._broadcastEmitter.emit('interactableUpdate', existingWhiteboardArea.toModel());
  }
  ...
  // public incrementLikes(id: number): number {
  //   // find an existing whiteboard area with the same ID
  //   const existingWhiteboardArea = <WhiteboardArea>(
  //     this._interactables.find(area => area instanceof WhiteboardArea)
  //   );

  //   const galleryCanvas = existingWhiteboardArea.gallery[id] as GalleryCanvas;
  //   return galleryCanvas?.incrementLikes();
  // }

  public getWhiteboardAreaComments(): Comment[] {
    const existingWhiteboardArea = <WhiteboardArea>(
      this._interactables.find(area => area instanceof WhiteboardArea)
    );
    return existingWhiteboardArea.comments;
  }

  public addWhiteboardAreaComment(comment: Comment): void {
    // find an existing whiteboard area with the same ID
    const existingWhiteboardArea = <WhiteboardArea>(
      this._interactables.find(area => area instanceof WhiteboardArea)
    );
    ...
    // we've reached here -- it's a valid update
    existingWhiteboardArea.addComment(comment);
    this._broadcastEmitter.emit('interactableUpdate', existingWhiteboardArea.toModel());
  }

  public clearWhiteboardAreaComments(): void {
    // find an existing whiteboard area with the same ID
    const existingWhiteboardArea = <WhiteboardArea>(
      this._interactables.find(area => area instanceof WhiteboardArea)
    );
  ...
  public initializeFromMap(map: ITiledMap) {
    ...
    const whiteboardArea = objectLayer.objects
      .filter(eachObject => eachObject.type === 'WhiteboardArea')
      .map(eachWBAreaObj => WhiteboardArea.fromMapObject(eachWBAreaObj, this._broadcastEmitter));

    this._interactables = this._interactables
      .concat(viewingAreas)
      .concat(conversationAreas)
      .concat(posterSessionAreas)
      .concat(whiteboardArea);
